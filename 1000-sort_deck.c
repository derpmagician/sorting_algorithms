...
/**
 * cardswap - swaps 2 consecutive cards of a doubly linked list
 * Used in the insertion algorithm
 * @deck: Head node for the deck
 * @card1: The first card to cardswap
 * @card2: The second card to cardswap
 */
void cardswap(deck_node_t **deck, deck_node_t *card1, deck_node_t *card2)
{
...
int vcmp(const card_t *card1, const card_t *card2)
{
	...
	const char *ord[14] = {"Ace", "1", "2", "3", "4", "5", "6",
...
/**
 * sort_suit - sorts a card deck by suit
 * @deck: deck to sort
 */
...
/**
 * sort_val - sorts a card deck staticly by value
 * @deck: deck to sort
 */
